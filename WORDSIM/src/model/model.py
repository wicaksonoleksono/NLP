...
class SkipGramModel:
    ...
    def forward(self, one_hot_vector):
        ...
        output_layer = self._softmax(output_layer)      # Shape: (vocab_size,)
    ...
    def backward(self, one_hot_vector, target_vector, learning_rate=0.1, clip_value=1.0):
        ...
        output_layer_gradient = np.outer(hidden_layer, error)  # Shape: (embedding_dim, vocab_size)
        hidden_layer_gradient = np.dot(self.W2, error)  # Shape: (embedding_dim,)
    ...
    def load(self, file_path):
        """Load the model's weights from a .npz file."""
